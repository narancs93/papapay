...
class PhoneNumberTest(BaseSetupTest):
    def test_create_and_save_model_with_owner_person(self):
    ...
    def test_create_and_save_model_with_owner_restaurant(self):
        phone_number = PhoneNumber(
            name='Landline Phone',
            country=self.country,
            phone_number='6059713696',
            owner_restaurant=self.restaurant)
        phone_number.save()
    ...
    def test_unique_country_and_phone_number_together_is_enforced(self):
        with self.assertRaises(IntegrityError):
            PhoneNumber.objects.create(
                country=self.country, phone_number='6059713695', owner_restaurant=self.restaurant)
